package com.github.kardzhaliyski.springbootclone.context;
...
import com.github.kardzhaliyski.springbootclone.context.annotations.*;
import com.github.kardzhaliyski.springbootclone.context.asyncs.AsyncDelegation;
import com.github.kardzhaliyski.springbootclone.context.events.EventListener;
import com.github.kardzhaliyski.springbootclone.context.events.*;
...
public class ApplicationContext {
    ...
    public <T> T getInstance(Class<T> c) throws Exception {
        ...
        if (initsInProgress.contains(clazzImpl)) {
            throw new ApplicationContextException("Circular dependency error. For class: " + clazzImpl);
        }
    ...
    private <T> Class<?> getClassImpl(Class<T> c) {
        ...
        if (clazzImpl == null && (c.isInterface() || isAbstract(c))) {
            throw new ApplicationContextException("No implementation found for Interface: " + c.getName());
        }
...
//            }
    ...
    private static Constructor<?> getConstructor(Class<?> clazz) throws NoSuchMethodException {
        ...
        for (Constructor<?> c : constructors) {
            ...
            if (constructor != null) {
                throw new ApplicationContextException("More then one constructor set with @Inject annotation. For class: " + clazz.getName());
            }
